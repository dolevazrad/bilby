# Plan: Control Run Experiment (Budget-Only, Broad Priors)

## What this experiment is and why

We have three run configurations:
- **Baseline**: nlive=2048, walks=100, broad priors → slow, good quality ✅
- **Phase 2**: nlive=1024, walks=50, compressed priors → fast, good quality ✅
- **Control** (NEW): nlive=1024, walks=50, **broad priors, no compression** → ?

The control answers the examiner question:
> "Is the speedup just from halving nlive, or does prior compression do real independent work?"

If the control gives poor quality (Δ_param > 1.0 or high BF variance), it proves that
nlive=1024 is only viable BECAUSE of prior compression — the compression is doing real work.

---

## IMPORTANT: Do NOT try to run the code

The paths in `all_in_one_correct_phase2.py` point to `/home/useradd/projects/bilby/...`
which does not exist on this machine. **That is fine.** The code will be copied to
the machine that has those paths. Your job is only to **write the Python files correctly**.
Do not attempt to execute anything. Do not report errors about missing paths.

---

## Step 1 — One change to `all_in_one_correct_phase2.py`
...
That is the only change needed in `all_in_one_correct_phase2.py`.
...
## What to send to the other machine

Copy these two files to the machine that has the ASD data:
1. `MyStuff/Phase_2/all_in_one_correct_phase2.py` (with the control branch added)
2. `MyStuff/Phase_2/run_control_experiment.py` (new file)
...
Once complete, report the final summary table printed to stdout, specifically:
- `control_mean_time_h` at 40 Mpc and 150 Mpc
- `budget_savings_pct` (how much does nlive alone save vs Baseline)
- `compression_savings_pct` (how much extra does Phase 2 save vs the control)
- Whether the control BF is consistent with Baseline
  (if yes: quality is preserved; if high variance: budget alone is insufficient)
